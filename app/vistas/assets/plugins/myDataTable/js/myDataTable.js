				...
				if (resA.data != undefined && resA.data.length > 0) {
					...
					for (var i = 0; i < resA.data.length; i++) {
						...
						for (var x = 0; x < data.colums.length; x++) {
							if(data.table.attr('id') === 'tablaProductos'){
								if (x == 0){
									var check = '';
									if(productos.includes(resA.data[i]['ID'])){
										check = 'checked';
									}

									columnas += '<td>' + resA.data[i][data.colums[x]]+ '<br><input class="form-check-input checkProducto" type="checkbox" '+check+'>' + '</td>';
								}else{
						...
						if (i == 0 && (data.table.attr('id') === 'tablaBuscarProductos' || data.table.attr('id') === 'tablaVentasPendientes' || data.table.attr('id') === 'tablaBuscarClientes')) {
							data.table.children('tbody').append('<tr id="' + resA.data[i]['ID'] + '" class="activa">' + columnas + '</tr>');
						}else{
