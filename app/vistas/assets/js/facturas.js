...
let c_cantidad = null;
let c_precio = null;
let c_descuento_porcentaje = null;
let c_descuento_dinero = null;
let c_subtotal = null;
let c_impuestos_total = null;
let c_total = null;

let imp_tasa = null;
let p_parcialidad = null;
let p_saldo_ant = null;
let p_monto_pagado = null;
let p_saldo_insoluto = null;
...
function v_facturas() {
  ...
  c_cantidad = IMask(
    document.getElementById('c_cantidad'),
    {
      mask: Number,
  ...
  c_precio = IMask(
    document.getElementById('c_precio'),
    {
      mask: Number,
      scale: 2,             // Decimales permitidos
      signed: false,        // Desactiva el signo negativo (-)
  ...
  c_descuento_porcentaje = IMask(
    document.getElementById('c_descuento_porcentaje'),
    {
      mask: Number,
      scale: 2,             // Decimales permitidos
      signed: false,        // Desactiva el signo negativo (-)
  ...
  c_descuento_dinero = IMask(
    document.getElementById('c_descuento_dinero'),
    {
      mask: Number,
      scale: 2,             // Decimales permitidos
      signed: false,        // Desactiva el signo negativo (-)
  ...
  c_subtotal = IMask(
    document.getElementById('c_subtotal'),
    {
      mask: Number,
      scale: 2,             // Decimales permitidos
      signed: false,        // Desactiva el signo negativo (-)
  ...
  c_impuestos_total = IMask(
    document.getElementById('c_impuestos_total'),
    {
      mask: Number,
      scale: 2,             // Decimales permitidos
      signed: false,        // Desactiva el signo negativo (-)
  ...
  c_total = IMask(
    document.getElementById('c_total'),
    {
      mask: Number,
      scale: 2,             // Decimales permitidos
      signed: false,        // Desactiva el signo negativo (-)
  ...
  imp_tasa = IMask(
    document.getElementById('imp_tasa'),
    {
      mask: Number,
      scale: 2,             // Decimales permitidos
      signed: false,        // Desactiva el signo negativo (-)
  ...
  p_parcialidad = IMask(
    document.getElementById('p_parcialidad'),
    {
      mask: Number,
      scale: 2,             // Decimales permitidos
      signed: false,        // Desactiva el signo negativo (-)
  ...
  p_saldo_ant = IMask(
    document.getElementById('p_saldo_ant'),
    {
      mask: Number,
      scale: 2,             // Decimales permitidos
      signed: false,        // Desactiva el signo negativo (-)
  ...
  p_monto_pagado = IMask(
    document.getElementById('p_monto_pagado'),
    {
      mask: Number,
      scale: 2,             // Decimales permitidos
      signed: false,        // Desactiva el signo negativo (-)
  ...
  p_saldo_insoluto = IMask(
    document.getElementById('p_saldo_insoluto'),
    {
      mask: Number,
      scale: 2,             // Decimales permitidos
      signed: false,        // Desactiva el signo negativo (-)
  ...
  p_imp_tasa = IMask(
    document.getElementById('p_imp_tasa'),
    {
      mask: Number,
      scale: 2,             // Decimales permitidos
      signed: false,        // Desactiva el signo negativo (-)
  ...
  $('#formConcepto').validate({
    rules: {
      ...
      c_cantidad: { required: true},
      c_precio: { required: true}
    },
    messages: {
      c_clave: { required: "La clave es requerida." },
      c_descripcion: { required: "La descripción es requerida." },
      c_cantidad: { required: "La cantidad es requerida." },
      c_precio: { required: "El precio es requerido." }
    },
    ...
    submitHandler: function (form) {
      ...
      let cantidad = parseFloat($('#c_cantidad').val().replaceAll(',', ''));
      let precio = parseFloat($('#c_precio').val().replaceAll(',', ''));
      let descuPorcen = parseFloat($('#c_descuento_porcentaje').val().replaceAll(',', '')) || 0;
      let descuento = parseFloat($('#c_descuento_dinero').val().replaceAll(',', '')) || 0;
...
function recalcularConcepto(origen = null) {
  let cantidad = parseFloat($('#c_cantidad').val().replaceAll(',', '')) || 0;
  let precio = parseFloat($('#c_precio').val().replaceAll(',', '')) || 0;

  let subtotal = cantidad * precio;

  let descuentoPorcentaje = parseFloat($('#c_descuento_porcentaje').val().replaceAll(',', '')) || 0;
  let descuentoDinero = parseFloat($('#c_descuento_dinero').val().replaceAll(',', '')) || 0;
  ...
  if (!bloqueando) {
    ...
    if (origen === 'porcentaje') {
      ...
      c_descuento_dinero.value = ''; 
      c_descuento_dinero.value = descuentoDinero.toFixed(2).toString();
    }

    if (origen === 'dinero') {
      descuentoPorcentaje = subtotal > 0 ? (descuentoDinero / subtotal) * 100 : 0;
      c_descuento_porcentaje.value = '';
      c_descuento_porcentaje.value = descuentoPorcentaje.toFixed(2).toString();
    }
  ...
  c_subtotal.value = '';
  c_subtotal.value = (subtotal - descuentoDinero).toFixed(2).toString();
  c_impuestos_total.value = '';
  c_impuestos_total.value = totalImpuestos.toFixed(2).toString();
  c_total.value = '';
  c_total.value = total.toFixed(2).toString();
  ...
  $.ajax({
    ...
    .done(function (res) {
      if ($.trim(res) === 'Correcto') {
        ...
        //tablaFacturas();
        $("#bRecargarVFacturas").trigger('click');
        $('#modalFactura').modal('hide');
      } else if ($.trim(res) === 'No hay timbres disponibles') {
        Swal.fire({
          icon: 'error',
          title: 'Error',
          text: 'Lo sentimos, no cuentas con timbres disponibles para timbrar la factura. Ponte en contacto con soporte para realizar la compra de timbres adicionales.'
        });
      }else {
        Swal.fire({
          icon: 'error',
...
function recalcularImpuestosPago() {
  let montoPagado = parseFloat($('#p_monto_pagado').val().replaceAll(',', '')) || 0;
...
jQuery(document).ready(function ($) {
  ...
  $(document).on('click', '#facturaGlobal', function () {
    if ($(this).prop('checked')) {
      ...
      $("#periodicidad").prop('required', true);   // ahora SÍ son requeridos porque se muestran
      ...
      $("#periodicidad").prop('required', false);  // ← corregido
      $("#mes").parent().addClass('d-none');
      $("#mes").prop('required', false);            // ← corregido
  ...
  $(document).on('click', '#btnAgregarImpuesto', function () {
    ...
    let valor = parseFloat($('#imp_tasa').val().replaceAll(',', '')) || 0;
    let tipoFactor = $('#imp_tipoFactor').val();
    let base = parseFloat($('#c_subtotal').val().replaceAll(',', '')) || 0;
  ...
  $(document).on('input', '.calcPago', function () {
    const saldoAnt = parseFloat($('#p_saldo_ant').val().replaceAll(',', '')) || 0;
    const montoPagado = parseFloat($('#p_monto_pagado').val().replaceAll(',', '')) || 0;
    let insoluto = saldoAnt - montoPagado;
    p_saldo_insoluto.value = '';
    p_saldo_insoluto.value = insoluto.toFixed(2).toString();
  });
  ...
  $(document).on('click', '#btnAgregarImpuestoPago', function () {
    ...
    let tasa = parseFloat($('#p_imp_tasa').val().replaceAll(',', '')) || 0;
    let montoPagado = parseFloat($('#p_monto_pagado').val().replaceAll(',', '')) || 0;
  ...
  $(document).on('submit', '#formDoctoRel', function (e) {
    ...
    const datos = {
      uuid: $('#p_uuid').val(),
      parcialidad: $('#p_parcialidad').val().replaceAll(',', ''),
      saldoAnt: parseFloat($('#p_saldo_ant').val().replaceAll(',', '')),
      montoPagado: parseFloat($('#p_monto_pagado').val().replaceAll(',', '')),
      saldoInsoluto: parseFloat($('#p_saldo_insoluto').val().replaceAll(',', '')),
