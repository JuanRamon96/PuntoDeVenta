...
function totalGastos() {
	if (new Date($("#fechaInicioVentas").val()) > new Date($("#fechaFinVentas").val())) {
		...
		var data = "metodo=consultar&accion=reportes&tipo=totalGastos&fechaInicio=" + $("#fechaInicioVentas").val() + "&fechaFin=" + $("#fechaFinVentas").val()
		+ "&usuario=" + $("#usuarioReporte").val() + "&caja=" + $("#cajaReporte").val();
		...
		$.ajax({
			...
			.done(function (res) {
				...
				$("#totalGastosValor").html(datos.TotalGastos);
				$("#gananciaVentasValor").html(datos.GananciaVentas);
				$("#diferenciaValor").html(datos.Diferencia);

				var diferenciaNum = parseFloat($(datos.Diferencia).text().replace(/[^0-9.-]+/g, ""));
				$("#diferenciaValor")
					.removeClass('text-success text-danger')
					.addClass(diferenciaNum >= 0 ? 'text-success' : 'text-danger');
...
function tablaVentasReportes() {
	ajaxMyDatatable({
		...
		params: {
			...
			usuario: $("#usuarioReporte").val(),
			caja: $("#cajaReporte").val()
		},
		totals: {
			4: "Costo",
			5: "Total",
...
function tablaGastosReportes() {
	ajaxMyDatatable({
		'table': $('#tablaGastosReportes'),
		'colums': [
			'Fecha',
			'Monto',
			'Descripcion',
...
jQuery(document).ready(function ($) {
	...
	$(document).on('change', '#cajaReporte', function () {
		totalGastos();
		tablaVentasReportes();
	});
});