let descuentoCompraDinero = null;
let importePagadoCobrar = null;

function v_hacerCompra() {
	$("#codigoProductoC").focus();

	descuentoCompraDinero = IMask(
		document.getElementById('descuentoCompraDinero'),
		{
			mask: Number,
	...
	importePagadoCobrar = IMask(
		document.getElementById('importePagadoCobrar'),
		{
			mask: Number,
			scale: 2,             // Decimales permitidos
			signed: false,        // Desactiva el signo negativo (-)
	...
	$('#formCobrarCompra').validate({
		...
		submitHandler: function (form) {
			if (parseFloat($("#restanteCobrar").text().replace('$', '').replace(searchRegExp, '')) < 0) {
				...
				data.append('descuento', $("#descuentoCompraDinero").val().replaceAll(',', ''));
				data.append('orden', $("#bGuardarOrden").attr('attrID'));

				data.set('importePagadoCobrar', importePagadoCobrar.unmaskedValue);
...
jQuery(document).ready(function ($) {
	...
	function calcularTotalCom() {
		...
		var descuento = parseFloat($("#descuentoCompraDinero").val().replaceAll(',', '')) ? parseFloat($("#descuentoCompraDinero").val().replaceAll(',', '')) : 0;
	...
	$(document).on('change keyup input', '#descuentoCompraDinero', function () {
		var descuento = parseFloat($(this).val().replaceAll(',', '')) ? parseFloat($(this).val().replaceAll(',', '')) : 0;
		var subtotal = $("#mostrarSubtotal").text().replace('$', '').replaceAll(',', '');
	...
	$(document).on('click', '#bRealizarCompra', function () {
		...
		if ($(this).attr("idProveedor") == "") {
			...
			if ($('#tipoCompra').val() == 'Contado') {
				...
				importePagadoCobrar.value = '';
				importePagadoCobrar.value = total.toString();
	...
	$(document).on('change keyup', '#importePagadoCobrar', function () {
		var importe = parseFloat($(this).val().replaceAll(',', '')) ? parseFloat($(this).val().replaceAll(',', '')) : 0;
		var total = parseFloat($("#totalCobrar").text().replace('$', '').replaceAll(',', ''));
	...
	$(document).on('click', '#bGuardarOrden', function () {
		if ($("#tbodyTablaProductosAgregados tr").length <= 0) {
			...
			var data = "metodo=modificar&accion=hacerCompra&tipo=" + tipo + "&total=" + $("#totalCompra").text().replace('$', '').replaceAll(',', '') + "&idProveedor=" + $("#bRealizarCompra").attr("idProveedor") 
			+ "&productos=" + JSON.stringify(productos) + "&descuento=" + $("#descuentoCompraDinero").val().replaceAll(',', '') + "&sucursal=" + $("#sucursalCompra").val() + "&id=" + $.trim($(this).attr('attrID'));
		...
		$.ajax({
			...
			.done(function (res) {
				...
				if (datos != null) {
					...
					descuentoCompraDinero.value = '';
					descuentoCompraDinero.value = datos.Descuento;
