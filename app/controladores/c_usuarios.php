...
class usuarios
{
  public function _consultar()
  {
    ...
    if (trim($buscar) != '') {
      ...
      for ($i = 0; $i < count($separa); $i++) {
        $busqueda .= "CONCAT(
          DATE_FORMAT(Fecha_Alta, '%d-%m-%Y %r'), 
          Nombre, 
          Primer_Apellido, 
          Segundo_Apellido, 
          Correo, 
          IF(Tipo_Usuario = 1, 'Administrador', IF(Tipo_Usuario = 0, 'Empleado', 'Tablet'))
        ) REGEXP '" . $separa[$i] . "'";
        if ($i < (count($separa) - 1)) {
          $busqueda .= ' AND ';
        }
    ...
    $query = "SELECT 
      ...
      Foto, 
      Fecha_Alta AS Fecha, 
      DATE_FORMAT(Fecha_Alta, '%d-%m-%Y %r') AS Fecha_Alta, 
      (SELECT COUNT(*) FROM usuarios WHERE ID_Usuario != '$idUsuario' $busqueda) AS Num 
    FROM usuarios WHERE ID_Usuario != '$idUsuario' AND ID_Usuario != '1' $busqueda ORDER BY $ordenColumna $orden LIMIT $limit OFFSET " . (($pagina * $limit) - $limit);
  ...
  public function _insertar()
  {
    ...
    $omodelo->_insertar('USE punto_venta_' . $_SESSION['user_punto_bd']);
    ...
    if ($error == 'si') {
      ...
      $rutaFotosUsuarios = 'vistas/assets/images/usuarios/';
      $nombreImg = '';
      $ruta = '';
      $rutaProvisional = '';
      if ($_FILES['fotoUsuario']['size'] > 0 && $_FILES['fotoUsuario']['error'] == 0) {
        ...
        if ($tipoImg != 'image/jpg' && $tipoImg != 'image/jpeg' && $tipoImg != 'image/png' && $tipoImg != 'image/gif') {
          ...
          $ruta = $rutaFotosUsuarios . $bd . '_' . $id . '_' . $nombreImg;
        }

        if ($status == 0 && $nombreImg != '') {
          $query = "UPDATE usuarios SET Foto = '" . $bd . '_' . $id . '_' . $nombreImg . "' WHERE ID_Usuario = '$id'";
          $error = $omodelo->_insertar($query);
  ...
  public function _modificar()
  {
    ...
    $omodelo->_insertar('USE punto_subs');
    $query = "UPDATE suscripciones SET 
      Nombre = '$nombre $primerApellido $segundoApellido',  
			Correo = '$correo', 
    WHERE Correo = (SELECT Correo FROM usuarios WHERE ID_Usuario = $id) AND BD = '" . $_SESSION['user_punto_bd'] . "'";
    $error = $omodelo->_insertar($query);
  ...
  public function _eliminar()
  {
    ...
    $omodelo->_insertar('USE punto_subs');
    $query = "DELETE FROM suscripciones WHERE Correo = (SELECT Correo FROM usuarios WHERE ID_Usuario = $id) AND BD = '" . $_SESSION['user_punto_bd'] . "'";
    $error = $omodelo->_insertar($query);
