<?php
  ...
  $queryCompra = "SELECT ID_Compra, ID_Compra AS Folio, FK_Usuario, FK_Sucursal, IFNULL((SELECT Nombre FROM sucursales WHERE ID_Sucursal = FK_Sucursal), '') AS Sucursal, FK_Proveedor, Razon_Social, RFC, Telefono, Email, Calle, No_Exterior, No_Interior, Colonia, CP, Ciudad, Estado, Pais, compras.Estatus AS Estatus, Descuento, Total, Tipo_Compra, CONCAT(usuarios.Nombre, ' ', usuarios.Primer_Apellido, ' ', usuarios.Segundo_Apellido) AS Nombre, DATE_FORMAT(compras.Fecha_Registro, '%d-%m-%Y %r') AS Fecha_Registro, DATE_FORMAT(Fecha_Credito, '%d-%m-%Y %r') AS Fecha_Credito, IFNULL((SELECT SUM(Monto) FROM compras_pagos WHERE FK_Compra = ID_Compra), 0) AS Pagado FROM compras INNER JOIN usuarios ON FK_Usuario = usuarios.ID_Usuario INNER JOIN proveedores ON FK_Proveedor = ID_Proveedor WHERE ID_Compra = '$id'";
  ...
  $queryDetalles = "SELECT ID_Detalle_Compra, FK_Compra, FK_Producto, Codigo, detalle_compras.Descripcion AS Descripcion, detalle_compras.Costo AS Costo, Cantidad, Subtotal FROM detalle_compras LEFT JOIN productos ON FK_Producto = ID_Producto WHERE FK_Compra = '$id'";
  ...
  $restante = 0; $afavor = 0;
  if(($rowCompra[0]['Total'] - $rowCompra[0]['Pagado']) > 0){
    $restante = $rowCompra[0]['Total'] - $rowCompra[0]['Pagado'];
  }else{
    $afavor = $rowCompra[0]['Pagado'] - $rowCompra[0]['Total'];
  }
  ...
  $tabla .= '</tbody>
    <tfoot>
      ...
      <tr>
        ...
        <td>$'.number_format($rowCompra[0]['Pagado'], 2).'</td>
      </tr>
      <tr>
        <th colspan="4" style="text-align: right;">Restante:</th>
        <td>$'.number_format($restante, 2).'</td>
      </tr>
      <tr>
        <th colspan="4" style="text-align: right;">A favor:</th>
        <td>$'.number_format($afavor, 2).'</td>
      </tr>
    </tfoot> 
  </table>';

  $fechaCredito = '';
  if($rowCompra[0]['Tipo_Compra'] != 'Contado'){
    $fechaCredito = '<p><b>Fecha Crédito: </b> '.$rowCompra[0]['Fecha_Credito'].'</p>';
  }
  ...
  <div style="font-family: Arial;">
    <div>
      ...
      <h1 style="margin-top: -45px; margin-left: 80px;">COMPRA</h1>
    </div>
    <div class="general">
    ...
    <div class="datos">
      ...
      <p><b>Tipo: </b> '.$rowCompra[0]['Tipo_Compra'].'</p>
