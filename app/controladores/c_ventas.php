        ...
        if ($numerofilas > 0) {
          for ($i = 0; $i < $numerofilas; $i++) {
            ...
            $arreglo['data'][$i] = array(
              ...
              'Cantidad' => '<span class="cantidad">' . $row[$i]['Cantidad'] . '</span>' . ($row[$i]["Devolucion"] > 0 ? '<br><span style="color: #a7090c;">(Dev: <b class="cantidad">' . $row[$i]["Devolucion"] . '</b>)</span>' : ''),
      ...
      if ($error == 'si') {
        ...
        if ($_FILES['comprobantePagoVenta']['size'] > 0 && $_FILES['comprobantePagoVenta']['error'] == 0) {
          ...
          if ($tipoImg != 'image/jpeg' && $tipoImg != 'image/jpg' && $tipoImg != 'image/png' && $tipoImg != 'application/pdf' && $tipoImg != '') {
            ...
            $ruta = $carpeta . $bd . '_' . $idPago . '_' . $nombreImg;
          }

          if ($status == 0 && $nombreImg != '') {
            $query = "UPDATE ventas_pagos SET Archivo = '" . $bd . '_' . $idPago . '_' . $nombreImg . "'  WHERE ID_Pago = '$idPago'";
            $error = $omodelo->_insertar($query);
            if ($error == 'si') {
