...
class proveedores {
  ...
  public function _consultar() {
    ...
    $query = "SELECT 
      ...
      (SELECT COUNT(*) FROM proveedores WHERE ID_Proveedor != 1 $busqueda) AS Num 
    ...
    if ($row == 'si') {
      ...
      if ($numerofilas > 0) {
        for ($i = 0; $i < $numerofilas; $i++) {
          ...
          if ($row[$i]['Credito'] != '') {
            $nombre .= '<br>Crédito: <b><span class="dinero">'.$row[$i]['Credito'].'</span></b><br>Adeudo: <b><span class="dinero">'.$row[$i]['Adeudo'].'</span></b><br>Restante: <b class="dinero">'.($row[$i]['Credito'] - $row[$i]['Adeudo']).'</b>';
          }
