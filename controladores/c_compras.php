...
class compras {
	...
	public function _consultar(){
		...
		if(trim($buscar) != ''){
			...
			for ($i=0; $i < count($separa); $i++) { 
				$busqueda .= "CONCAT(DATE_FORMAT(compras.Fecha_Registro, '%d-%m-%Y %r'), ID_Compra, Razon_Social, RFC, proveedores.Email, proveedores.Telefono, Total, Estatus, Tipo_Compra, IFNULL((SELECT Nombre FROM sucursales WHERE FK_Sucursal = ID_Sucursal), 'NA')) REGEXP '".$separa[$i]."'";
				if($i < (count($separa)-1)){
					$busqueda .= ' AND ';
				}
			}
		...
		$query = "SELECT ID_Compra, ID_Compra AS Datos, FK_Sucursal, IFNULL((SELECT Nombre FROM sucursales WHERE FK_Sucursal = ID_Sucursal), 'NA') AS Sucursal, FK_Usuario, (SELECT usuarios.Nombre FROM usuarios WHERE ID_Usuario = FK_Usuario) AS NombreUsuario, FK_Proveedor, RFC, proveedores.Email AS Email, proveedores.Telefono AS Telefono, Razon_Social, Tipo_Compra, Total, Estatus, compras.Fecha_Registro AS Fecha, DATE_FORMAT(compras.Fecha_Registro, '%d-%m-%Y %r') AS Fecha_Registro, IFNULL((SELECT SUM(Monto) FROM compras_pagos WHERE FK_Compra = ID_Compra), 0) AS Pagado, (SELECT COUNT(*) FROM compras INNER JOIN proveedores ON FK_Proveedor = ID_Proveedor $busqueda) AS Num FROM compras INNER JOIN proveedores ON FK_Proveedor = ID_Proveedor $busqueda ORDER BY $ordenColumna $orden LIMIT $limit OFFSET ".(($pagina * $limit) - $limit);
	...
	public function _insertar(){
		...
		$importePago=  $omodelo->link->real_escape_string($importePago);
