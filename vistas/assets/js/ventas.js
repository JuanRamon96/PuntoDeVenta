...
function totalModiVenta() {
  ...
  var porcentaje = parseFloat($("#porcentajeDescuentoModiVenta").val()) || 0;
  var descuento = Math.round(((porcentaje / 100) * subtotal) * 100) / 100;
  $("#cantidadDescuentoModiVenta").val(descuento);
  var total = subtotal - descuento;
  $("#totalModiVenta").text(total);
  var pago = parseFloat($("#pagoModiVenta").val()) || 0;
...
$(document).ready(function () {
  ...
  $(document).on('click', '.bModificarVenta', function () {
    ...
    $.ajax({
      ...
      $("#cantidadDescuentoModiVenta").val(datos.Descuento);
      $("#porcentajeDescuentoModiVenta").val(Math.round(((parseFloat(datos.Descuento) / subtotal) * 100) * 100) / 100);
      $("#pagoModiVenta").val(datos.Pago);
  ...
  $(document).on('change keyup', '#cantidadDescuentoModiVenta', function () {
    ...
    $('#porcentajeDescuentoModiVenta').val(porcentaje);
    $('#totalModiVenta').text(total);
    ...
    if ($("#metodoPagoModiVenta").val() == "Crédito" && $("#clienteModiVen").attr('attrID') == 0) {
      ...
      Swal.fire({
        ...
        if (result.value) {
          ...
          var datos = "metodo=detalles&accion=ventas&tipo=modificarVenta&cliente=" + $("#clienteModiVen").attr('attrID') + "&descuento=" + $("#cantidadDescuentoModiVenta").val() + "&total=" + $("#totalModiVenta").text().replace('$', '').replaceAll(',', '') + "&pago=" + $('#pagoModiVenta').val() + "&cambio=" + $("#cambioModiVen").text().replace('$', '').replaceAll(',', '') + "&tipoPago=" + $('#metodoPagoModiVenta').val() + "&detalles=" + $.trim($('#detallesModiVenta').val()) + "&id=" + btn.attr('attrID') + "&productos=" + JSON.stringify(productos);
